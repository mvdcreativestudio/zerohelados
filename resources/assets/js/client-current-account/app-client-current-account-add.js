$(document).ready(function() {
  ...
  function submitNewCurrentAccount() {
    ...
    if (!$('#client_id').val() || !$('#current_account_settings_id').val() || !$('#currency_id_current_account').val()) {
      Swal.fire({
        icon: 'error',
    ...
    $.ajax({
      ...
      success: function (response) {
        Swal.fire({
          ...
          window.location.href = `${baseUrl}admin/current-account-client-payments/${response.id}`;
        });
      },
      error: function (xhr) {
        var errorMessage =
